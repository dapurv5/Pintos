...
process_execute (const char *file_name)
{
  ...
  /*My Implementation*/
  palloc_free_page(my_fn_copy);   /*Free the memory allocated*/
  /*My Implementation*/

  return tid;

  /*Change struct thread parameters related to child*/
}
...
start_process (void *file_name_)
{
  ...
  if(success){
    ...
    cur->image_on_disk=filesys_open(file_name);
    cur->load_status=1;
    file_deny_write(cur->image_on_disk);
    push_arguments(&if_.esp,my_fn_copy);
  }
  ...
  if (!success){
      ...
      cur->load_status=-1;
      cur->exit_status=-1;
      sema_up(&cur->wait);        /*If some parent was blocked on this thread free that parent. This seems to be redundant*/
      /*My Implementation*/
      thread_exit ();
  }
...
process_exit (void)
{
  ...
  int n=list_size(&cur->open_file_list);
  ...
  //Remove this thread from the list of its parent
  if(cur!=initial_thread){
      list_remove(&cur->child_elem);
      file_close(cur->image_on_disk);
      cur->image_on_disk=NULL;
  }


    sema_up(&cur->wait);
    sema_init (&cur->zombie, 0);
    sema_down (&cur->zombie);//Don't let the child thread be destroyed because parent thread needs to get some information from it.
